...
IMAGE_R6D_LOCAL="/grdata/services/offline/rainbond.images.upgrade.5.1.0.tgz"
IMAGE_BASE_LOCAL="/grdata/services/offline/base.images.upgrade.5.1.0.tgz"

IMAGE_PATH="/grdata/services/offline/upgrade"

INSTALL_SCRIPT="/grdata/services/offline/rainbond-ansible.upgrade.5.1.0.tgz"
...
if [ -f "$INSTALL_SCRIPT" ];then
    mv /opt/rainbond/rainbond-ansible /opt/rainbond/rainbond-ansible_5.0.4
    tar xf ${INSTALL_SCRIPT} -C /opt/rainbond
    rm -rf /opt/rainbond/rainbond-ansible/inventory
    cp -a /opt/rainbond/rainbond-ansible_5.0.4/inventory /opt/rainbond/rainbond-ansible
    secretkey=$(cat /opt/rainbond/rainbond-ansible_5.0.4/roles/rainvar/defaults/main.yml | grep secretkey | awk '{print $2}')
    db_pass=$(cat /opt/rainbond/rainbond-ansible_5.0.4/roles/rainvar/defaults/main.yml | grep db_pass | awk '{print $2}')
    pod_cidr=$(cat /opt/rainbond/rainbond-ansible_5.0.4/roles/rainvar/defaults/main.yml | grep pod_cidr | awk '{print $2}')
    app_domain=$(cat /opt/rainbond/rainbond-ansible_5.0.4/roles/rainvar/defaults/main.yml | grep app_domain | awk '{print $2}')
    default_dns_local=$(cat /opt/rainbond/rainbond-ansible_5.0.4/roles/rainvar/defaults/main.yml | grep default_dns_local | awk '{print $2}')
