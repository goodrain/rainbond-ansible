...
{
    c:\rainbond\kubectl.exe --kubeconfig=c:\rainbond\config get nodes/$($(hostname).ToLower())
    return (!$LASTEXITCODE)
}
...
function RegisterNode()
{
    if (!(IsNodeRegistered))
    {
        $argList = @("--hostname-override=$(hostname)","--pod-infra-container-image=kubeletwin/pause","--resolv-conf=""""", "--cgroups-per-qos=false", "--enforce-node-allocatable=""""","--kubeconfig=c:\rainbond\config")
        $process = Start-Process -FilePath c:\rainbond\kubelet.exe -PassThru -ArgumentList $argList
...
"@
    ...
    $ipamConfig | Out-File "C:\rainbond\sourceVipRequest.json"

    $env:CNI_COMMAND="ADD"
    $env:CNI_CONTAINERID="dummy"
    $env:CNI_NETNS="dummy"
    $env:CNI_IFNAME="dummy"
    $env:CNI_PATH="c:\rainbond\cni" #path to host-local.exe
...
{
    return c:\rainbond\kubectl.exe --kubeconfig=c:\rainbond\config get nodes/$($(hostname).ToLower()) -o custom-columns=podCidr:.spec.podCIDR --no-headers
}

function Get-PodCIDRs()
{
    return c:\rainbond\kubectl.exe  --kubeconfig=c:\rainbond\config get nodes -o=custom-columns=Name:.status.nodeInfo.operatingSystem,PODCidr:.spec.podCIDR --no-headers
}
